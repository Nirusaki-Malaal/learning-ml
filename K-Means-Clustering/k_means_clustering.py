...
class Kmeans():
    def __init__(self,X, cluster, epoch=1000):
        self.X = np.array(X)
        self.K = cluster
    ...
    def cost(self,c, u):
        loss = np.sum(np.linalg.norm(self.X - u[c], axis=1)**2)
        return loss 
    ...
    def train(self, verbose=True):
        for epoch in range(self.epoch):
            ...
            distances = np.linalg.norm(self.X[:, np.newaxis] - self.centroids,axis=2) ## check this line later
            C = np.argmin(distances, axis=1) ## check this line later
            self.C = C
            ## centroid update
